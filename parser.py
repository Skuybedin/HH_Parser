...
vacancy = {'менеджер': 'https://hh.ru/resumes/menedzher?page=', 'помощник': 'https://hh.ru/resumes/pomoschnik-rukovoditelya?page='}

""" Начало работы """
print('*** Выберите желаемую вакансию для парсинга ***')
vacancy_parser = input('Менеджер, Помощник : ').lower()
print(f"*** Введите кол-во страниц для парсинга вакансии '{vacancy_parser}' ***")
number_of_pages = int(input('Кол-во страниц: '))

""" Переменные для SQL """
table_name = {'менеджер': 'hh_menedjer', 'помощник': 'hh_pomoschnik'}
...
def ftp_client(password):
    file_name = table_name[vacancy_parser] + '.csv'
    ftp = ftplib.FTP('37.140.192.51')
    ftp.login('u1330875', password)
    with open("C:\\Users\\Никита\\Desktop\\Parser\\" + file_name, 'rb') as upload_file:
        ftp.storbinary('STOR ' + '/www/skuybedin.ru/' + file_name, upload_file)
...
if 'yes' in create_csv_question or 'y' in create_csv_question:
	...
	upload_question = input('Напишите yes/no: ')
	if 'yes' in upload_question or 'y' in upload_question:
		ftp_client(input('Введите пароль от FTP: '))